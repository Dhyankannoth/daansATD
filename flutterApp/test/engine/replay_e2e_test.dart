...
import 'package:pulseguard/engine/core/config/thresholds.dart' show AssetBundleLike;
...
void main() {
  test('reaction.json (with demo baseline) reaches high between t=50s and t=110s '
      'and opens exactly one check-in', () async {
    final clock = FakeClock();
    final engine = PulseGuardEngine(clock: clock, bundle: _FileBundle(), alertEffects: _NoopAlertEffects());
    ...
    await engine.startReplay('assets/traces/reaction.json', speed: 100, useDemoBaseline: true);
  ...
  test('normal.json (with demo baseline) never exceeds monitoring', () async {
    ...
    await engine.startReplay('assets/traces/normal.json', speed: 100, useDemoBaseline: true);
    final summary = await engine.scanFinished.first;
  ...
  test('signal_loss.json opens a check-in (multi_system or signal_loss) and never '
      'crashes on null vitals', () async {
    final clock = FakeClock();
    final engine = PulseGuardEngine(clock: clock, bundle: _FileBundle(), alertEffects: _NoopAlertEffects());
    await engine.initialize();
    ...
    await engine.startReplay('assets/traces/signal_loss.json',
        speed: 100, useDemoBaseline: true);
    final summary = await engine.scanFinished.first;
    ...
    expect(
      ...
      anyOf(EscalationTriggerType.multiSystem, EscalationTriggerType.signalLoss),
    );
