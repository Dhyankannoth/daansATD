...
import 'package:pulseguard/engine/detection/fusion_engine.dart' show FusionResult;
...
DeviationFlag _flag(BodySystem s, {bool deviating = false, bool trusted = true, String reason = ''}) {
...
void main() {
  ...
  test('a currently-deviating flag (not yet persistent) -> monitoring', () {
    final engine = RiskEngine(thresholds: _load());
    final flags = [
      _flag(BodySystem.cardiovascular, deviating: true, reason: 'HR 20% above baseline'),
      _flag(BodySystem.respiratory),
      _flag(BodySystem.autonomic),
    ];
    final r = engine.evaluate(
  ...
  test('ML alone can reach elevated (sustained anomaly) but never high, even with '
