...
const _mb72 = MetricBaseline(
    mean: 72, sd: 5, variance: 25, sdFloorApplied: true, sessionCount: 3, updatedAt: 0);
const _mb45 = MetricBaseline(
    mean: 45, sd: 8, variance: 64, sdFloorApplied: true, sessionCount: 3, updatedAt: 0);
const _mb14 = MetricBaseline(
    mean: 14, sd: 2, variance: 4, sdFloorApplied: true, sessionCount: 3, updatedAt: 0);
const _baseline = Baseline(hr: _mb72, hrv: _mb45, rr: _mb14, isDemo: false);

VitalsReading _reading({double? hr, double? hrv, double? rr, double quality = 0.9}) {
...
void main() {
  test('HR beyond hr_z SDs above baseline is flagged deviating at rest', () {
    ...
    final trends = Trends(windowS: 60, trendMinPoints: 30, mlMetricMinPoints: 20);
    ...
    final cardio = flags.firstWhere((f) => f.system == BodySystem.cardiovascular);
  ...
  test('recovering activity suppresses HR deviation when HR is falling', () {
    final engine = DeviationEngine(thresholds: _load());
    final trends = Trends(windowS: 60, trendMinPoints: 2, mlMetricMinPoints: 20);
